  ...
  (syntax-parse stx
    [(define-endpoint name:id
     ...
     (with-syntax* ([request-id (format-id #'name "request-~a" #'name)]
                    ...
                    [out-id (format-id #'name "*~a-out*" #'name)]
       ...
       #`(begin
           ...
           (define out-id '#,(append
                              (flatten
                               (map (lambda (obj)
                                      (define struct-name (hash-ref obj 'id))
                                      (cons struct-name
                                            (map (lambda (field-detail)
                                                   (format "~a-~a" (syntax->datum struct-name)
                                                           (hash-ref field-detail 'name)))
